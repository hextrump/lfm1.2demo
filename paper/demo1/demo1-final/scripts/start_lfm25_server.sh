#!/usr/bin/env bash
# Start llama.cpp server with LFM2.5 1.2B Instruct.
...
MODEL_ALIAS="lfm2.5-1.2b-instruct"
...
if [ -n "${LFM25_GGUF_PATH:-}" ] && [ -f "${LFM25_GGUF_PATH}" ]; then
  ...
  for candidate in \
    "${DEMO_DIR}/lfm2.5-1.2b-instruct-q4_k_m.gguf" \
    "${DEMO_DIR}/models/lfm2.5-1.2b-instruct-q4_k_m.gguf" \
    "${DEMO_DIR}/../lfm2.5-1.2b-instruct-q4_k_m.gguf"; do
    if [ -f "${candidate}" ]; then
